...
#
#   It takes a while to crawl the relevant parts of CRAN, so a bunch of stuff
#   here is written so that if needed things can be restarted after interruption
#   without loosing progress.
#

setwd(here::here("news"))
...
# Now, ID RStudio-related packages as well, using the DESCRIPTION info
root_url <- "https://cran.r-project.org/package="

# Initialize rstudio var, if needed (if we restart the loop, it might already
# exist)
if (is.null(cran_packs$rstudio)) cran_packs$rstudio <- NA_integer_
