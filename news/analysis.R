...
dl <- readRDS(here("news/data/downloads.rds"))
...
ggsave("img/cran-downloads.png", height = 5, width = 8)
...
#
#   Write cleaned table for flexdashboard ----
...
table_data <- by_pack %>%
  select(package, resid, rstudio, days_up, total_count, med_daily_count, avg_daily_count) %>%
  rename(Package = package,
         Score = resid,
         RStudio = rstudio,
         `Days up` = days_up,
         `Total dl` = total_count,
         `Median dl` = med_daily_count,
         `Mean dl` = avg_daily_count)

table_data <- table_data %>%
  arrange(desc(Score)) %>%
  mutate(Score = round(Score, 2),
         RStudio = as.integer(RStudio),
         `Days up` = as.integer(`Days up`),
         `Total dl` = as.integer(`Total dl`),
         `Median dl` = as.integer(`Median dl`),
         `Mean dl` = round(`Mean dl`, 0))

write_csv(table_data, here::here("news/data/new-packages.csv"))
